...
// parse uptime environmental variables
...
//if checked we use port 445 if no port specified, if unchecked, port 80 if no port specified.
   $portvalue = strlen(getenv('UPTIME_PORT'));
if ( empty($portvalue) || strlen(getenv('UPTIME_PORT')) == "") {
	if (getenv('UPTIME_USE-SSL') == "true") {
		$port = 445;
	}else {
		$port = 80;
...
print $errorstr."\r\n";
print $timeout."\r\n";
print 'STREAM_CLIENT_CONNECT'."\r\n";
print $context."\r\n";
print $ssl."\r\n";
	//try {
		if ($ssl) {
			$context = stream_context_create(array(
 			       'ssl' => array('SNI_server_name' => $hostname),
			    ));
			
			$resource = stream_socket_client('ssl://'.$host.":".$port, $errorno, $errorstr, $timeout,STREAM_CLIENT_CONNECT,$context);
		} else {
